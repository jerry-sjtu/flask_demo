  ...
  Store.prototype = {
    ...
    _organiseData: function(dataPoint, forceRender) {
      ...
      if (!store[x][y]) {
        store[x][y] = value;
        radi[x][y] = radius;
        //console.log(dataPoint,store[x][y]);
      } else {
        store[x][y] += value;
        //console.log(dataPoint,store[x][y]);
      }
 ...
 */
 ...
 (function() {
    ...
    getMapData = function (data, _this) {
      var d = data,
      dlen = d.length;
      ret = [];
      var currentBounds = _this._map.getBounds();
      while (dlen--) {
        ...
        var divPixel = _this._map.pointToOverlayPixel(latlng),
        leftX = _this._map.pointToOverlayPixel(currentBounds.getSouthWest()).x,
        topY = _this._map.pointToOverlayPixel(currentBounds.getNorthEast()).y,
        screenPixel = new BMap.Pixel(divPixel.x - leftX, divPixel.y - topY);
        var point = _this.pixelTransform(screenPixel);
    ...
    averageMapData = function (data,avg_r,radius) {
      sum = [];
      count = [];
      var r = avg_r;
      for (var i=0 ; i<data.length; i++) {
        ...
        sum[x][y]++;
        count[x][y] += v;
        for (var tx=Math.max(x-r,0); tx<=x+r ; tx++) {
          if (!sum[tx]) {
            sum[tx] = [];
            count[tx] = [];
          }
          for (var ty=Math.max(y-r,0); ty<=y+r; ty++) {
            if (tx==x && ty==y) continue; //skip itself
            if (!sum[tx][ty]) {
              sum[tx][ty] = 0;
              count[tx][ty] = 0;
            }
     ...
     */
     HeatmapOverlay.prototype.setAvgDataSet = function(data) {
        this.data = data;
        if(!isSupportCanvas()){//判断是否支持Canvas.
          return ;
        }
        ...
        var d = data.data,
        dlen = d.length,
        avg_r = data.avg_r*zoomFactor(this._map.getZoom()),
        radius = data.radius;
        ...
        allMapData = getMapData(d,this);
        mapdata.data = averageMapData(allMapData,avg_r,radius);
    ...
    /**
     * 设置两个数据源的差值热力图展现的详细数据, 实现之后,即可以立刻展现 
     * @param {Json Object } data
     * {"<b>max</b>" : {Number} 权重的最大值,
     * {"<b>min</b>" : {Number} 权重的最小值,
     * <br />"<b>positive</b>" : {Array} 坐标详细数据,格式如下 <br/>
     * {"lng":116.421969,"lat":39.913527,"count":3},
     * <br />"<b>negative</b>" : {Array} 坐标详细数据,格式如下 <br/>
     * {"lng":116.421969,"lat":39.913527,"count":3}
     * lng lat分别为经纬度, count权重值
     */
     HeatmapOverlay.prototype.setAvgDifferenceDataSet = function(data) {
      this.data = data;
        if(!isSupportCanvas()){//判断是否支持Canvas.
          return ;
        }
        ...
        positiveMapData = averageMapData(
                              getMapData(data.positive,this),
                              data.avg_r,
                              data.radius);
        negativeMapData = averageMapData(
                              getMapData(data.negative,this),
                              data.avg_r,
                              data.radius);

        for (var i=0 ; i<positiveMapData.length; i++) {
          mapdata.data.push({
            x:positiveMapData[i].x,
            y:positiveMapData[i].y,
            count: positiveMapData[i].count,
            radius: positiveMapData[i].radius
          });
        }

        for (var i=0 ; i<negativeMapData.length; i++) {
          mapdata.data.push({
            x:negativeMapData[i].x,
            y:negativeMapData[i].y,
            count: -negativeMapData[i].count,
            radius: negativeMapData[i].radius
          });
        }
        this.heatmap.setData(mapdata);
      }
     ...
     * lng lat分别为经纬度, count权重值
     */
     HeatmapOverlay.prototype.setDifferenceDataSet = function(data) {
      this.data = data;
        if(!isSupportCanvas()){//判断是否支持Canvas.
          return ;
        }
        ...
        positiveMapData = getMapData(data.positive,this);
        negativeMapData = getMapData(data.negative,this);
